package net.com.diagnose.ldn.LDNetDiagnoService;
...
import net.com.diagnose.ldn.LDNetDiagnoUtils.LDNetUtil;
 ...
 *
 *
 * Created by selva on 2018/10/12.
 *
 */
public class LDNetDiagnoService extends
  ...
  public LDNetDiagnoService(Context context, String theAppCode,
                            ...
                            String theDeviceID, String theDormain, String theCarrierName,
                            String theISOCountryCode, String theMobileCountryCode,
                            String theMobileNetCode, LDNetDiagnoListener theListener, Logurl logurl) {
    super();
    this._context = context;
    this._appCode = theAppCode;
    this._appName = theAppName;
  ...
  public String startNetDiagnosis() {
    ...
    if (_isNetConnected) {
     ...
     // _traceRouter.startTraceRoute(_dormain);
      logurl.setLogDetail(logDetail);
      System.out.println("::<><>>>>>>:"+logurl.getUrl());
      Contans.addList(logurl);
      logDetail.setUrl(logurl.getUrl());
      Contans.generaterLog(logDetail);
      /*LogFileStore filest =MainActivity.logfil;
      if(filest!=null){
           filest.writeObject(logurl);
      }*/
...
//    recordStepInfo("用户id:\t" + _UID);
  ...
  private void recordLocalNetEnvironmentInfo() {
    ...
    this.logDetail.setNetworkType(_netType);
    if (_isNetConnected) {
     ...
     this.logDetail.setServerIPAddress(_localIp);
    } else {
    ...
    if (_isNetConnected) {
      ...
      this.logDetail.setDns(this._dns1+","+this._dns2);
    } else {
  ...
  private boolean parseDomain(String _dormain) {
    ...
    if (_remoteInet != null) {// 解析正确
      ...
      logDetail.setServerIPAddress(ipString);
      flag = true;
    ...
    this.logDetail.setRuniingTime(useTime+"ms");
    return flag;
  }
